// FilesView.swift - duplicate & large file finder pane (ported from FilesApp.swift).
// Finder-style: click to select, Space for Quick Look, ↑↓ to move (also while
// the preview panel is open).
    ...
    func autoSelectOlder() {
        for g in dupGroups.indices {
            for i in dupGroups[g].files.indices {
                dupGroups[g].files[i].selected = (i != 0)
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Picker("", selection: $model.tab) {
                    ...
                    Text("Duplicates" + (model.dupGroups.isEmpty ? "" :
                        "  (\(fmtBytes(model.totalWasted)) wasted)")).tag(1)
                }
                    ...
                    if model.tab == 0 {
                        ...
                        ForEach($model.dupGroups) { $group in
                            VStack(alignment: .leading, spacing: 0) {
                                Text("\(group.files.count) copies · \(fmtBytes(group.size)) each · \(fmtBytes(group.wasted)) wasted")
            ...
            HStack {
                ...
                Spacer()
                Text("Selected: \(model.selectedCount) files · \(fmtBytes(model.selectedTotal))")
        ...
        .onDisappear {
            // don't let Space/arrows act on this pane while another pane is showing
            model.selectedPath = nil
            model.previewURL = nil
        }
        .confirmationDialog(
            "Move \(model.selectedCount) files (\(fmtBytes(model.selectedTotal))) to Trash?",
